...
def main2():
    ...
    with open("db_builder/data/words_lists/webnext.txt", "w") as f:
        f.write("\n".join(words))
...
def main3():
    ...
    with open("db_builder/data/words_lists/quizlet_voltaire.txt", "w") as f:
        words = []
...
def main4():
    BASE_URL = "https://www.laculturegenerale.com/ameliorer-vocabulaire-enrichir/"
    ...
    for item in items.findChildren("li" , recursive=False):
        strong = item.find("strong")
        words.append(strong.text.strip().strip(":").strip())

    with open("db_builder/data/words_lists/culture_generale.txt", "w") as f:
        f.write("\n".join(words))

def main5():
    BASE_URL = "https://www.francoisenore.com/articles/mots-rares-0"
    ...
    for i in range(1, 3):
        ...
        assert response.ok
        soup = BeautifulSoup(response.text, "html.parser")
        items = soup.find("div", class_="articles_index_contenu_article")

        for item in items.find_all("strong"):
            words.append(item.text)
        for item in items.find_all("b"):
            if item.text:
                words.append(item.text.strip())

    with open("db_builder/data/words_lists/francoisnore.txt", "w") as f:
        f.write("\n".join(words))

def main6():
    BASE_URL = "https://jolismots.fr/dictionnaire/#"
    ...
    assert response.ok
    soup = BeautifulSoup(response.text, "html.parser")
    items = soup.find("main", id="main")

    for item in items.find_all("li", class_="worditem"):
        words.append(item.text)

    with open("db_builder/data/words_lists/jolismots.txt", "w") as f:
        f.write("\n".join(words))
